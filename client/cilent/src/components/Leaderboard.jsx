...
const Leaderboard = () => {
  ...
  const fetchLeaderboard = useCallback(async (isRefresh = false) => {
    try {
      ...
      // Detect new entries (not in previous list)
    ...
    <div className="min-h-screen bg-slate-950 text-white overflow-x-hidden">
      ...
      <nav className="flex justify-between items-center px-6 md:px-8 py-5 border-b border-slate-800/60 sticky top-0 bg-slate-950/90 backdrop-blur-md z-50">
        ...
        <div className="flex items-center gap-3">
          ...
          {user && (
            <div className="hidden sm:flex items-center gap-2 text-sm text-gray-300 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2">
      ...
      >
        ...
        <p className="mt-4 text-gray-400 text-lg max-w-2xl mx-auto">
          Powered by Redis Sorted Sets — rankings update the instant a score
          lands.
        </p>
      </motion.section>
      ...
      <main className="max-w-5xl mx-auto px-6 pb-20">
        <div className="grid lg:grid-cols-[1fr_340px] gap-8 items-start">
          ...
          >
            {/* Score submission */}
            <ScorePanel onScoreSubmitted={handleScoreSubmitted} />
